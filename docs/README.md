# Ptolemy Docs

Use this page as the documentation hub for Ptolemy.

## Start Here

- [Setup](./Setup.md)
- [CLI Guide](./CLI.md)
- [Worker API](./Worker_API.md)
- [Development Workflow](./Development.md)
- [Architecture](./Architecture.md)
- [Project Memory](./memory/projects/ptolemy)

## KB And Agent Loop

- Start KB-first with `.ptolemy/PTOLEMY.md`, then `.ptolemy/kb/PROJECT_MAP.md`, then the KB indexes before broad repo reads.
- Use [Worker API](./Worker_API.md) for the HTTP surfaces, including the controller-owned `/agent-runs` loop.
- Use [Workflow Index](../WORKFLOWS.md) when the task needs the repo's execution model rather than just endpoint reference.

## Task System

- [Task System Overview](./tasks/README.md)
- [Task-File Driven Workflow](./workflows/agent/task-file-driven.md)
- [Task Pack Execution Workflow](./workflows/agent/task-pack-execution.md)
- [Multi-Task Execution](./workflows/multi-task-execution.md)
- [Task Pack Template](./tasks/templates/task-pack-template/README.md)

## Workflow Index

- [Workflow Index](../WORKFLOWS.md)
- [Core Workflows](./workflows/core)
- [Agent Workflows](./workflows/agent)
- [Editing Workflows](./workflows/editing)
- [Recovery Workflows](./workflows/recovery)
- [Git Workflows](./workflows/git)

## Design And Planning

- [MVP Design](./plans/MVP_Design.md)
- [Build Plan](./plans/Build Plan.md)
- [Future Updates](./plans/Future Updates.md)
- [Worker Progress Checklist](./plans/Worker_Progress_Checklist.md)

## Memory

- [Project Memory](./memory/projects/ptolemy)
