// Local copy of github.com/alphacep/vosk-api/go v0.3.50 (see vendor-notes.md in
// this directory). Only the three cgo flag lines differ from upstream: they point
// at the repo's own vosk-lib/ directory instead of ${SRCDIR}/../src.
//
// These paths are set here as package-scoped cgo directives rather than via the
// global CGO_CPPFLAGS / CGO_LDFLAGS environment variables on purpose: those env
// vars are applied to EVERY cgo package the build touches, including Go's own
// runtime/cgo (which compiles with -Werror), and the leaked include path makes
// that build die with "runtime/cgo: ... exit status 2" before it ever reaches
// Vosk. ${SRCDIR} is third_party/vosk-go, so ${SRCDIR}/../../vosk-lib resolves to
// <repo-root>/vosk-lib (machine-independent).
package vosk
